...
def ft(args, result, config):
    ...
    print("best ACC is", max(clean_accuracies), "and the epoch is", clean_accuracies.index(max(clean_accuracies)))
    print("the corresponding ASR is", ASRs[clean_accuracies.index(max(clean_accuracies))])
    print("min ASR is", min(ASRs))
    logging.info('Best Test Acc: {:.3f}%'.format(best_ACC))
    logging.info('Best Test Asr: {:.3f}%'.format(best_ASR))
    # import matplotlib.pyplot as plt
    # plt.hlines(original_ACC, 0, args.epochs, colors="green", label="original ACC")
    # plt.hlines(original_ASR, 0, args.epochs, colors="red", label="original ASR")
    # plt.plot(range(args.epochs), clean_accuracies, label="ACC")
    # plt.plot(range(args.epochs), ASRs, label="ASR")
    # plt.title("finetuning results")
    # plt.legend()
    # figure_name = os.path.join(os.getcwd() + args.checkpoint_save, "finetuning"+str(args.ratio)+".png")
