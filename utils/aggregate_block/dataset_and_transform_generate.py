...
from utils.dataset.bd_dataset import xy_iter
...
def get_dataset_normalization(dataset_name):
    ...
    if dataset_name == "cifar10":
        ...
        dataset_normalization = transforms.Normalize([0.489, 0.409, 0.372], [1, 1, 1])
...
def get_dataset_norm_stats(dataset_name):
    if dataset_name == "cifar10":
        ...
        return ([0.489, 0.409, 0.372], [1, 1, 1])
    else:
        raise Exception("Invalid Dataset")
...
def get_transform(dataset_name, input_height, input_width, train=True, norm=True):
...
def speed_up_save(
    ...
    all_x_numpy = np.concatenate(x_list) if len(x_list)>0 else np.array([])
...
def split_dataset_VGGFace2(dataset, val_frac, shuffle=True):
    val_indexes = []
    train_indexes = []

    unique_class, count_class = np.unique(dataset.y, return_counts=True)
    for i in range(len(unique_class)):
        class_label = unique_class[i]
        class_indexes = np.nonzero(dataset.y == class_label)[0]
    ...
    val_set.X = [dataset.X[i] for i in val_indexes]
    val_set.y = [dataset.y[i] for i in val_indexes]
    val_set.split = 'val'
    val_set.image_list = [dataset.image_list[i] for i in val_indexes]
    val_set.length = len(val_set.y)

    train_set = dataset.copy()
    train_set.X = [dataset.X[i] for i in train_indexes]
    train_set.y = [dataset.y[i] for i in train_indexes]
    train_set.image_list = [dataset.image_list[i] for i in train_indexes]
    train_set.length = len(train_set.y)
...
def dataset_and_transform_generate(args):
    ...
    if (train_dataset_without_transform is None) or (test_dataset_without_transform is None):
        ...
        if args.dataset.startswith('test'): # for test only
            ...
            from utils.dataset.VGGFace2 import VGGFace2_Dataset
            train_dataset_without_transform = VGGFace2_Dataset(args.dataset_path,
                                                          split='train')
            train_dataset_without_transform.build()
            test_dataset_without_transform = VGGFace2_Dataset(args.dataset_path,
                                                         split = 'test')
            test_dataset_without_transform.build()
            args.val_frac = 0
            train_dataset_without_transform, val_dataset_without_transform = split_dataset_VGGFace2(
