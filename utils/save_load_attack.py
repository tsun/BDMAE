...
def summary_dict(input_dict):
    ...
    for k, v in input_dict.items():
        ...
        elif (isinstance(v, torch.Tensor) and v.numel()>0 ) or (isinstance(v, np.ndarray) and v.size>0 ):
...
def load_attack_result(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        clean_setting.dataset_path = os.path.join('./data', load_file['data_path'].split('/')[-1])
        ...
        if load_train:
            ...
            clean_train_x = clean_train_y = None
        ...
        if load_val:
            ...
            clean_val_x = clean_val_y = None

        if (load_file['bd_train']['x'] is not None) and (load_file['bd_train']['y'] is not None) and (load_file['bd_train']['original_index'] is not None) and load_train:
